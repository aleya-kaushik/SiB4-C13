...
!...misc values
integer(i4) :: n,s,nref,tcref,isorefc13
integer(i4) :: isorefc13nt
...
do n=1,npoolpft/2 !1,5
    ...
    isorefc13nt=n+ntpool/2 !ntpool 12,16
...
!    poollt%poolpftmin(n) = pool_valid
    ...
    do s=1,pool_indx_lay(n) !1,5 ntpool, indx_lay same for isorefc13
       ...
       !poollt%loss_mresp_lay(isorefc13,s) = fract%rcpoolfac*temp_mrespr
       !poollt%poolpft_dloss(isorefc13,s) = fract%rcpoolfac*temp_mrespl & !(npoolpft,nsoil)
       !    + poollt%poolpft_dloss(isorefc13,s)
!       temp_mresprc13 = poollt%poolpft_lay(isorefc13,s) * &
!                     poollt%krater_lay(isorefc13,s) * &
!                     poolcont%lresp_eff(isorefc13)
!       temp_mresplc13 = temp_mrespr*dtsib
       ...
       !if ( (poollt%loss_mresp_lay(isorefc13,s) .gt. 1) .or. &
       !     (poollt%loss_mresp_lay(isorefc13,s) .lt. -1) ) then
